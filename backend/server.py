...
async def update_media(media_id: str, payload: MediaUpdate, user: dict = Depends(get_current_user)):
    ...
    # synchronize attachments immediately for every linked item
    for li in media_doc.get("linked_items", []):
        item_type = li.get("item_type")
        item_id = li.get("item_id")
        col = ITEM_COLLECTIONS.get(item_type)
        if col is None or not item_id:
            continue

        if "preview_on_card" in update_data:
            if bool(update_data["preview_on_card"]):
                # one preview ON: set all false first, then this media true
                await col.update_one(
                    {"id": item_id, "user_id": user["id"]},
                    {"$set": {"attachments.$[].preview_on_card": False}},
                )
                await db.media_items.update_many(
                    {
                        "user_id": user["id"],
                        "id": {"$ne": media_id},
                        "linked_items": {"$elemMatch": {"item_type": item_type, "item_id": item_id}},
                    },
                    {"$set": {"preview_on_card": False, "updated_at": datetime.now(timezone.utc).isoformat()}},
                )

            await col.update_one(
                {"id": item_id, "user_id": user["id"]},
                {"$set": {"attachments.$[a].preview_on_card": bool(update_data["preview_on_card"])}},
                array_filters=[{"$or": [{"a.id": media_id}, {"a.media_id": media_id}]}],
            )
...
@api_router.post("/media/{media_id}/toggle-preview")
async def toggle_media_preview(media_id: str, user: dict = Depends(get_current_user)):
    media_doc = await db.media_items.find_one({"id": media_id, "user_id": user["id"]})
    if not media_doc:
        raise HTTPException(status_code=404, detail="Media non trouve")

    next_preview = not bool(media_doc.get("preview_on_card", False))
    now_iso = datetime.now(timezone.utc).isoformat()

    await db.media_items.update_one(
        {"id": media_id, "user_id": user["id"]},
        {"$set": {"preview_on_card": next_preview, "updated_at": now_iso}},
    )

    impacted = set()
    for li in media_doc.get("linked_items", []):
        t, i = li.get("item_type"), li.get("item_id")
        if t in ITEM_COLLECTIONS and i:
            impacted.add((t, i))

    for t, col in ITEM_COLLECTIONS.items():
        docs = await col.find(
            {"user_id": user["id"], "$or": [{"attachments.id": media_id}, {"attachments.media_id": media_id}]},
            {"_id": 0, "id": 1},
        ).to_list(2000)
        for d in docs:
            impacted.add((t, d["id"]))

    for item_type, item_id in impacted:
        col = ITEM_COLLECTIONS[item_type]
        item_doc = await col.find_one({"id": item_id, "user_id": user["id"]}, {"_id": 0, "attachments": 1})
        old_atts = (item_doc or {}).get("attachments", [])
        att_ids = set((a.get("media_id") or a.get("id")) for a in old_atts if (a.get("media_id") or a.get("id")))

        medias = await db.media_items.find({"user_id": user["id"]}, {"_id": 0}).sort("created_at", -1).to_list(5000)

        filtered = []
        for m in medias:
            mid = m.get("id")
            lis = m.get("linked_items") or []
            linked_here = any(li.get("item_type") == item_type and li.get("item_id") == item_id for li in lis)
            if linked_here or mid in att_ids:
                filtered.append(m)
        ...
        for m in filtered:
            ...
            if next_preview:
                ...
                if mid == media_id:
                    ...
                    pv = bool(m.get("preview_on_card", False)) and not seen
            if pv:
                seen = True

            await db.media_items.update_one(
                {"id": mid, "user_id": user["id"]},
                {"$set": {"preview_on_card": pv, "updated_at": now_iso}},
            )
            ...
            new_atts.append({
                ...
                "original_name": m.get("original_name") or m.get("title") or m.get("url") or "media",
                "mime_type": m.get("mime_type") or "application/octet-stream",
                "size": m.get("size", 0),
                "url": m.get("url"),
                "preview_on_card": pv,
                "uploaded_at": m.get("created_at") or now_iso,
            })

        await col.update_one(
            {"id": item_id, "user_id": user["id"]},
            {"$set": {"attachments": new_atts, "updated_at": now_iso}},
        )
...
async def delete_media(media_id: str, user: dict = Depends(get_current_user)):
    ...
    # Remove stale attachments everywhere + fix preview fallback
    for col in ITEM_COLLECTIONS.values():
        ...
        for d in docs:
            old = d.get("attachments") or []
            new = [a for a in old if (a.get("id") or a.get("media_id")) != media_id]

            if new and not any(bool(x.get("preview_on_card")) for x in new):
                new[0]["preview_on_card"] = True
                first_id = new[0].get("id") or new[0].get("media_id")
                if first_id:
                    await db.media_items.update_one(
                        {"id": first_id, "user_id": user["id"]},
                        {"$set": {"preview_on_card": True, "updated_at": datetime.now(timezone.utc).isoformat()}},
                    )
